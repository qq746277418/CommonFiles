...
function MyPokerList:_getCardIdxByTouch(tPos)
	local cardIdx = nil
	for idx,x in pairs(self.m_dPosList) do
		if tPos.x < x then
			cardIdx = idx
			break
		end
...
function MyPokerList:_onCardVTouchBegan(tPos)
	...
	if idx then
		...
		self:_updatePokersPositionCenter(idx)
...
function MyPokerList:_updatePokersPosition()
	...
	for _,poker in pairs(self.m_cPokers) do
		local pos = self:_getPosBySortId(_)
		poker:move(pos)
		poker:addBeganPos(pos)
		poker:setLocalZOrder(_)
		local dis = _ == #self.m_cPokers and self.m_dCardWidth or self.m_dCardDis
		self.m_dPosList[_] = (pos.x - self.m_dCardWidth / 2 + dis)
	end
...
function MyPokerList:_updatePokersPositionCenter(centerId)
	local dis = self.m_dCardWidth / 2
	local disNum = 6

	if self.m_dCardDis >= dis then
		return
	end
	self.m_dCardDis = (self.m_dTotalWidth - self.m_dCardWidth - dis * 6) / (#self.m_cPokers - 7)
	local cenNum = 0
	for _,poker in pairs(self.m_cPokers) do
		local pos = self:_getPosBySortId(_)
		if _ >= centerId - 2 and _ <= centerId + 3 then
			cenNum = cenNum + 1
		end

		pos.x = pos.x + dis * cenNum
		poker:move(pos)
		poker:addBeganPos(pos)
		poker:setLocalZOrder(_)
		local dis = _ == #self.m_cPokers and self.m_dCardWidth or self.m_dCardDis
